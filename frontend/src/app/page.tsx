...
export default function Home() {
  ...
  // Lock body scroll while guest memory modal is open
  useEffect(() => {
    if (!showGuestMemoryModal) return;
    const originalOverflow = document.body.style.overflow;
    document.body.style.overflow = 'hidden';
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setShowGuestMemoryModal(false);
    };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-blue-600 selection:text-white">
      ...
      {showGuestMemoryModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200 overflow-y-auto">
          <div className="w-full max-w-md max-h-[90vh] flex flex-col my-auto overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl p-6 sm:p-7 animate-in zoom-in-95 duration-150">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 py-3 text-sm text-slate-300 pr-1">
